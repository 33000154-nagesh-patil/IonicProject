...
export class SignatureUploadComponent implements OnInit {
  ...
  constructor(private router:Router,private camera: Camera, private networkService: NetworkService, private allConfigDataService: AllConfigDataService, private actionSheetController: ActionSheetController, public modalCtrl: ModalController, private commonService: CommonService, private loaderService: LoaderService, private http: HttpClient, private commonFunctionService: CommonFunctionService, private cdn: ChangeDetectorRef) { }
  ...
  getAPI(file) {
    ...
    formData.append('CustWetSign', file);
    formData.append('CustGuId', this.loginCustomerGuId);

    let headers: HttpHeaders = new HttpHeaders({
      "Token": this.environmentAPIList?.token,
    });
    this.http.post(this.apiCatalog.baseURL[this.appEnviron]+this.breadCrumb+this.apiCatalog.wetSign, formData)
