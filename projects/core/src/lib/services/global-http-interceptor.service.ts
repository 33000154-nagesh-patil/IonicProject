...
export class GlobalHttpInterceptorService {
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    // console.log("this.environmentAPIList?.bseStar", this.environmentAPIList?.bseStar)

    const token: string = (req.url == this.environmentAPIList?.bseStar)?'fMqHkvwLKoN6rTyt_j7F3HNgnvhBtWWE':'Bearer invald token';
