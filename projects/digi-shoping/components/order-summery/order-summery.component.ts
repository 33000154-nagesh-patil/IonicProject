...
export class OrderSummeryComponent implements OnInit {
  ...
  fetchProductionTransaction(obj) {
    ...
    if (obj) {
      ...
      if (this.grandTotal > 0) {
        this.payWithRazor1(this.grandTotal, obj?.OrderGuId);
        // let bseParam = {
        //   "clientCode": "TDPL009",
  ...
  payWithRazor1(amount, orderId) {
    ...
    var options = {
      ...
      prefill: {
        email: localStorage.getItem('EmailId'), //optional
        contact: localStorage.getItem('SocialClientId'), //optional
        name: '' //optional
      },
      theme: {
        color: '#003399'
  ...
  myfun(val) {
    console.log("sgddfhfdghfgjfghjgfjfdhfdybrtyryjtguksrhtebugj5yqjayesh");
    ...
    this.loaderService.showLoader();
    this.http.post(this.apiCatalog.baseURL[this.appEnviron]+this.breadCrumb+this.apiCatalog.buyDigiGold,param).pipe(retry(3)).subscribe(async (data: any) => {

      if (data && data?.message?.split(" ")[0] == "Successfully") {
        console.log("update", data);
        this.transectionId = data.result?.data?.transactionId
