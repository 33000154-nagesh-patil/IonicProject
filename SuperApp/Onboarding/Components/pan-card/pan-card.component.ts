...
import { civilKraDetails, panDetails } from 'index';
...
// })
  ...
  processToNextStep(obj) {
    ...
    if(localStorage.getItem("userType") =="Partner"){
       ...
       Url=this.apiCatalog.baseURL[this.apiCatalog.environment]+this.apiCatalog.breadCrumb+this.apiCatalog.panAadhaarSeeding
    }
    ...
    .subscribe((data: any) => {
      ...
      if (data.Status==='1') {
        this.loaderService.hideLoader();
        if(data.data){
          this.commonService.userPan.next(data.data);
          this.panDetailsData = data.data;
          this.panUserName = this.panDetailsData.Name.concat(
            ' ',this.panDetailsData.MiddleName.concat(
              ' ',this.panDetailsData.LastName.concat()
              ));
          this.commonService.userPanName.next(this.panUserName);
        }else{
          let obj = {
            "PanNo": this.currentInputPan
          }
          this.commonService.userPan.next(obj);
        }
        if(localStorage.getItem("userType") =="Partner"){
        this.router.navigate(['/Onboarding'+data['pageUrl']]);
        }else{
          // this.router.navigate(['/Onboarding'+data['pageUrl']]);
          this.onboardingService.nextOnSuccess('PAN');
        }
      } else {
        this.loaderService.hideLoader();
        this.showNextStep = false;
        // this.onboardingService.skip('PAN');
        this.errorShow(data?.Message, "processToNextStep -> status");
        // onboardingStep.data[objIndex+1]
      }
    }, (error: any) => {
      this.showNextStep = false;
      // this.onboardingService.skip('PAN');
  ...
  skip(val){
    ...
    //   if(x.OnboardingSteps=="PAN")objIndex=x.Id;
    //   if(objIndex&&x.Id==objIndex+1)this.router.navigate(['/Onboarding'+x['pageUrl']]);
    // }
  ...
  clickDisclaimerURL() {
    ...
    window.open(urlFetch.disclaimerURL);
  }
  clickTncUrl() {
    // this.commonFunctionService.inAppBrowser(urlFetch.tncURL);
    window.open(urlFetch.tncURL);
  }
