...
export class SelfieVerificationComponent implements OnInit {
  ...
  constructor(private router:Router,private camera: Camera, private geolocation: Geolocation,
    private toastService: ToastService, private actionSheetController: ActionSheetController,
    private http: HttpClient, private loaderService: LoaderService,
    private allConfigDataService: AllConfigDataService, private cdn: ChangeDetectorRef, private commonFunctionService: CommonFunctionService,
    private onboardingService:OnboardingService,
    public modalCtrl: ModalController, private networkService: NetworkService, ) {
  ...
  ngOnInit() {
    ...
    this.breadCrumbPartner='Onboarding/OnboardingSteps/PartnerSelfie';
...
let todayDate = dd + '/' + mm + '/' + yyyy;
  ...
  if(localStorage.getItem("userType") =="Partner"){
    Url=this.apiCatalog.baseURL[this.appEnviron]+this.breadCrumb+this.apiCatalog.Selfie
  }
    ...
    .subscribe(
      (data: any) => {
        ...
        this.onboardingService.nextOnSuccess('Selfie')
  ...
  looksGood() {
    if(localStorage.getItem('userType') == 'Partner'){
      this.router.navigate(['Onboarding/WetSignature'])
      return;
    }
    // this.getSelfieStatus.emit("Selfie")
